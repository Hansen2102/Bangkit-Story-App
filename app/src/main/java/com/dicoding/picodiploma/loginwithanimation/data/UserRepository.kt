...
class UserRepository private constructor(
    ...
    fun userStories(token: String): LiveData<PagingData<ListStoryItem>> {
        return Pager(
            config = PagingConfig(
                pageSize = 20
            ),
    ...
    suspend fun uploadImage(file: MultipartBody.Part,token: String,description: RequestBody): FileUploadResponse {
        return apiService.uploadImage(file,token,description)
    }

    suspend fun getUserStoriesLocation(token:String): StoryResponse {
